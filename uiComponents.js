...
function createControlField(id, label, modifierClass = '') {
  const container = document.createElement('div')
  container.className = `control-group ${modifierClass}`.trim()
...
export function addSlider(id, label, min, max, value, onChange) {
  const { container } = createControlField(id, label)
...
export function addColorPicker(id, label, defaultColor, onChange) {
  const { container } = createControlField(id, label, 'control-group--color')
  const valueElement = document.createElement('span')
  valueElement.className = 'control-value'
  valueElement.textContent = defaultColor.toUpperCase()
  ...
  picker.addEventListener('input', e => {
    valueElement.textContent = e.target.value.toUpperCase()
    onChange(e.target.value)
  })
